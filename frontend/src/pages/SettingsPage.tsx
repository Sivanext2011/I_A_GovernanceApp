    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {FILE_TYPES.map(({ key, label, desc }, idx) => (
          <motion.div key={key} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.1 }} className="glass-card p-6">
            ...
            <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-slate-300 dark:border-slate-600 rounded-xl cursor-pointer hover:border-primary-400 transition-colors">
              ...
              <span className="text-sm text-[var(--text-secondary)]">{uploading === key ? 'Uploading...' : 'Click to upload .xlsx / .xls'}</span>
              <input type="file" accept=".xlsx,.xls" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f) handleUpload(key, f) }} disabled={uploading === key} />
            </label>
            {results[key] && <p className={`text-xs mt-2 ${results[key].success ? 'text-green-600' : 'text-red-500'}`}>{results[key].message}</p>}
          </motion.div>
        ))}
      </div>
      ...
      <div className="glass-card p-6">
        ...
        <p className="text-xs text-[var(--text-secondary)] mb-4">
          Paste an access token from <a href="https://developer.microsoft.com/en-us/graph/graph-explorer" target="_blank" rel="noreferrer" className="text-primary-500 underline">Graph Explorer</a> (ensure Mail.Send permission is consented).
        </p>
        <div className="flex gap-3">
          <input type="password" value={token} onChange={(e) => setToken(e.target.value)} placeholder="Paste access token here..." className="flex-1 px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-[var(--bg-secondary)] text-sm" />
          <button onClick={handleSetToken} disabled={tokenSaving || !token.trim()} className="btn-primary flex items-center gap-2 text-sm disabled:opacity-50">
            <Key className="w-4 h-4" /> {tokenSaving ? 'Saving...' : 'Set Token'}
          </button>
          <button onClick={checkAuth} className="btn-secondary text-sm">Check Status</button>
        </div>
        {authStatus !== null && <p className={`text-sm mt-3 ${authStatus ? 'text-green-600' : 'text-amber-600'}`}>{authStatus ? '✓ Authenticated' : '✗ Not authenticated'}</p>}
      </div>
...
function SavingsOverrideManager() {
  ...
  const setMutation = useMutation({
    ...
    onSuccess: () => { refetch(); queryClient.invalidateQueries(); setFeedbackId(''); setReuseSaving(''); setAutomationSaving('') },
  })
  ...
  return (
    <div className="glass-card p-6">
      ...
      <p className="text-xs text-[var(--text-secondary)] mb-4">Update Reuse Saving and Automation Saving for specific Feedback IDs. Persists across re-uploads.</p>
      <div className="flex gap-3 mb-4 flex-wrap">
        <input value={feedbackId} onChange={(e) => setFeedbackId(e.target.value)} placeholder="Feedback ID" className="px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-[var(--bg-secondary)] text-sm w-40" />
        <input value={reuseSaving} onChange={(e) => setReuseSaving(e.target.value)} placeholder="Reuse Saving" type="number" step="0.01" className="px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-[var(--bg-secondary)] text-sm w-36" />
        <input value={automationSaving} onChange={(e) => setAutomationSaving(e.target.value)} placeholder="Automation Saving" type="number" step="0.01" className="px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-[var(--bg-secondary)] text-sm w-36" />
        <button onClick={() => setMutation.mutate({ feedback_id: feedbackId.trim(), reuse_saving: parseFloat(reuseSaving) || 0, automation_saving: parseFloat(automationSaving) || 0 })} disabled={!feedbackId.trim()} className="btn-primary flex items-center gap-2 text-sm disabled:opacity-50">
          ...
          <tbody>
            {overrides.map((o: any) => (
              <tr key={o.feedback_id} className="border-b border-[var(--border)]">
                ...
                <td className="py-2"><button onClick={() => removeMutation.mutate(o.feedback_id)} className="text-red-500 hover:text-red-700"><XCircle className="w-4 h-4" /></button></td>
              </tr>
            ))}
          </tbody>
        </table>
...
function ExclusionManager() {
  ...
  return (
    <div className="glass-card p-6">
      ...
      <p className="text-xs text-[var(--text-secondary)] mb-4">Records added here will be excluded from all calculations, even after re-uploading data.</p>
      <div className="flex gap-3 mb-4">
        <input value={patId} onChange={(e) => setPatId(e.target.value)} placeholder="PAT ID(s) to exclude (comma-separated)" className="flex-1 px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-[var(--bg-secondary)] text-sm" />
        <button onClick={() => addPatMutation.mutate(patId.split(',').map(s => s.trim()).filter(Boolean))} disabled={!patId.trim()} className="btn-primary flex items-center gap-2 text-sm disabled:opacity-50">
          <Plus className="w-4 h-4" /> Exclude PAT
        </button>
      </div>
      <div className="flex gap-3 mb-6">
        <input value={feedbackId} onChange={(e) => setFeedbackId(e.target.value)} placeholder="Feedback ID(s) to exclude (comma-separated)" className="flex-1 px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-[var(--bg-secondary)] text-sm" />
        <button onClick={() => addFeedbackMutation.mutate(feedbackId.split(',').map(s => s.trim()).filter(Boolean))} disabled={!feedbackId.trim()} className="btn-primary flex items-center gap-2 text-sm disabled:opacity-50">
            ...
            <div className="space-y-1 max-h-40 overflow-y-auto">
              {exclusions.pat_ids?.map((id: string) => (
                <div key={id} className="flex items-center justify-between text-xs bg-slate-50 dark:bg-slate-800 px-3 py-1.5 rounded">
                  <span>{id}</span>
                  <button onClick={() => removePatMutation.mutate([id])} className="text-red-500 hover:text-red-700"><XCircle className="w-3.5 h-3.5" /></button>
                </div>
              ))}
          ...
          <div>
            ...
            <div className="space-y-1 max-h-40 overflow-y-auto">
              {exclusions.feedback_ids?.map((id: string) => (
                <div key={id} className="flex items-center justify-between text-xs bg-slate-50 dark:bg-slate-800 px-3 py-1.5 rounded">
                  <span>{id}</span>
                  <button onClick={() => removeFeedbackMutation.mutate([id])} className="text-red-500 hover:text-red-700"><XCircle className="w-3.5 h-3.5" /></button>
                </div>
              ))}
